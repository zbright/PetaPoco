...
namespace PetaPoco
{
    ...
    public class Database
    {
        ...
        static void AddParam(IDbCommand cmd, object item, string paramName)
        {
            ...
            if (args.Length > 0)
            {
                foreach (var item in args)
                {
                    if ((item as IEnumerable<string>) != null || (item as IEnumerable<int>) != null)
                    {
                        ...
                        {
                            AddParam(result, parm, _paramPrefix + origCount + "p" + newCount);
                            newCount++;
                        }

                        var newParams = Enumerable.Range(0, newCount).Select(i => _paramPrefix + origCount + "p" + i).ToArray();
