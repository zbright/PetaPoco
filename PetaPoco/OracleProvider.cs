...
namespace PetaPoco
{
    ...
    public interface IOracleProvider {
        IDbConnection CreateConnectionOracle();
        IDbCommand CreateCommandOracle();
    }

    public class OracleProvider : DbProviderFactory, IOracleProvider
    {
        ...
        public IDbConnection CreateConnectionOracle()
        {
            // Oracle Connnection doesn't implement DbCommand before Oracle Client 10.2 rel2
            return (IDbConnection)(Activator.CreateInstance(_connectionType));
        }

        public IDbCommand CreateCommandOracle()
        {
            IDbCommand command = (IDbCommand)Activator.CreateInstance(_commandType);
